...
class ControllerPaymentPAYMENTNEPAL extends Controller {
	public function index() {
		...
		$data['key'] = $this->config->get('paymentnepal_key');
		...
		$data['name'] = $this->config->get('paymentnepal_name');

		$data['email'] = $order_info['email'];

		$data['order_id'] = $order_info['order_id']; 
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paymentnepal.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/paymentnepal.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/paymentnepal.tpl', $data);
		}
	}
    ...
    $ch = join('', array_values($request)) . $this->config->get('paymentnepal_secret') ;
    echo "test2-".$ch ;
		$check = md5(join('', array_values($request)) . $this->config->get('paymentnepal_secret'));
    echo "test-".$check ;   
    ...
    print_r($request) ;
		...
		if (isset($this->request->post['tid'])) {
			...
			if (isset($this->request->post['check']) && ($this->request->post['check'] == $check)) {
				$this->model_checkout_order->addOrderHistory($this->request->post['order_id'], $this->config->get('paymentnepal_order_status_id'), $message, false);
			} else {
	...
	/*		$data['continue'] = $this->url->link('checkout/success');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paymentnepal_success.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/paymentnepal_success.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/payment/paymentnepal_success.tpl', $data));
			} */
		} else {
			$data['continue'] = $this->url->link('checkout/cart');

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paymentnepal_failure.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/paymentnepal_failure.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/payment/paymentnepal_failure.tpl', $data));
			}
		}
	}    
